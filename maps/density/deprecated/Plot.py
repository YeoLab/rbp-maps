...
def single_frame_with_up_down_events_error(up, down, both,
    ...
    min1, max1 = plot_err(ax, 'region1', up, down, both, uperr, downerr, upcolor, downcolor, bgcolor, uplab, downlab,
                          bglab)
...
def two_frame_with_inclusion_exclusion_events_with_error(inclusion, exclusion, both,
    ...
    with dataviz.Figure(output_file, figsize=(num_cols * 4, num_rows * 4)) as fig:
        ...
        min1, max1 = plot_err(ax1, 'region1', inclusion, exclusion, both, inclusion_err, exclusion_err, color1, color2,
                              color3)
        ...
        min2, max2 = plot_err(ax2, 'region2', inclusion, exclusion, both, inclusion_err, exclusion_err, color1, color2,
                              color3)
...
def four_frame_with_inclusion_exclusion_events_with_error(inclusion, exclusion, both,
    ...
    min3, max3 = plot_err(ax3, 'region3', inclusion, exclusion, both, inclusion_err, exclusion_err, color1, color2,
                          color3)
    ax3.set_yticklabels([])
    ...
    min4, max4 = plot_err(ax4, 'region4', inclusion, exclusion, both, inclusion_err, exclusion_err, color1, color2,
                          color3)
...
def six_frame_with_inclusion_exclusion_events_with_error(inclusion, exclusion, both,
    ...
    with dataviz.Figure(output_file, figsize=(num_cols * 4, num_rows * 4)) as fig:
        ...
        min5, max5 = plot_err(ax5, 'region5', inclusion, exclusion, both, inclusion_err, exclusion_err, color1, color2,
                              color3)
        ax5.set_yticklabels([])
        ...
        min6, max6 = plot_err(ax6, 'region6', inclusion, exclusion, both, inclusion_err, exclusion_err, color1, color2,
                              color3)
        ax6.set_yticklabels([])
...
def plot_mxe(inclusion, exclusion, both, inclusion_err, exclusion_err, title, output_file,
    ...
    Split the single region into four. This will break if the regions are of different dimensions!!!
    """
    i['region1'], i['region2'], i['region3'], i['region4'], i['region5'], i['region6'] = np.array_split(
        inclusion['region1'], 6)
    e['region1'], e['region2'], e['region3'], e['region4'], e['region5'], e['region6'] = np.array_split(
        exclusion['region1'], 6)
    b['region1'], b['region2'], b['region3'], b['region4'], b['region5'], b['region6'] = np.array_split(both['region1'],
                                                                                                        6)
    ie['region1'], ie['region2'], ie['region3'], ie['region4'], ie['region5'], ie['region6'] = np.array_split(
        inclusion_err['region1'], 6)
    ee['region1'], ee['region2'], ee['region3'], ee['region4'], ee['region5'], ee['region6'] = np.array_split(
        exclusion_err['region1'], 6)
