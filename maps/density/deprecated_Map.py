...
class ClipWithInput(Map):
    ...
    def __init__(
            self, ReadDensity, InputReadDensity, output_file,
            name, is_scaled=False,
            annotation=None,
            annotation_type="miso",
            left=0, right=0,
    ...
    def normalize(self, normfunc=norm.read_entropy, min_density_sum=0,
                  label=""):
        """
        ...
        for feature in self.ip_raw_density:
            ...
            self.density[feature].to_csv(
                "{}.{}.{}.normed_matrix.csv".format(self.output_base, label,
                                                    feature))
    ...
    def set_means_and_sems(self, feature, conf=0.95):
        ...
        """
        self.logger.info(
            "Start removing outliers - {} (conf={})".format(self.name, conf))
        means = list()
        sems = list()
        ...
        self.sems = sems
        self.logger.info(
            "Finish removing outliers - {} (conf={})".format(self.name, conf))

    def create_matrices(self, label=""):
        densities = [self.ip_raw_density, self.input_raw_density]
        rbps = [self.ip, self.inp]
        self.logger.info(
            "Start creating the Matrix - {} (scaled={})".format(self.name,
                                                                self.is_scaled))
        for i in range(0, len(densities)):
            densities[i]['feature'] = mtx.exon(annotation=self.annotation,
                                               density=rbps[i],
                                               upstream_offset=0,
                                               downstream_offset=0,
        ...
        self.logger.info(
            "Finished creating the Matrix - {} (scaled={})".format(self.name,
                                                                   self.is_scaled))
        self.ip_raw_density['feature'].to_csv(
            "{}.ip.{}_raw_density_matrix.csv".format(self.output_base, label))
        self.input_raw_density['feature'].to_csv(
            "{}.input.{}_raw_density_matrix.csv".format(self.output_base,
                                                        label))
        self.maptype = label
    ...
    def create_a3ss_matrices(self, label=""):
        ...
        rbps = [self.ip, self.inp]
        self.logger.info(
            "Start creating the A3SS Matrix - {}".format(self.name))
        for i in range(0, len(densities)):
            densities[i]['feature'] = mtx.alt_3p_splice_site(
        ...
        self.logger.info(
            "Finished creating the A3SS Matrix - {}".format(self.name))
        self.ip_raw_density['feature'].to_csv(
            "{}.ip.{}.{}.a3ss.raw_density_matrix.csv".format(self.output_base,
                                                             label, 'feature'))
        self.input_raw_density['feature'].to_csv(
            "{}.input.{}.{}.a3ss.raw_density_matrix.csv".format(
                self.output_base, label, 'feature'))
        self.maptype = 'a3ss'
    ...
    def create_a5ss_matrices(self, label=""):
        ...
        rbps = [self.ip, self.inp]
        self.logger.info(
            "Start creating the A5SS Matrix - {}".format(self.name))
        for i in range(0, len(densities)):
            densities[i]['feature'] = mtx.alt_5p_splice_site(
                annotation=self.annotation,
                annotation_type=self.annotation_type,
                density=rbps[i],
                exon_offset=self.exon_offset,
                ...
                combine_regions=True)
        self.logger.info(
            "Finished creating the A5SS Matrix - {}".format(self.name))
        self.ip_raw_density['feature'].to_csv(
            "{}.ip.{}.{}.a5ss.raw_density_matrix.csv".format(self.output_base,
                                                             label, 'feature'))
        self.input_raw_density['feature'].to_csv(
            "{}.input.{}.{}.a5ss.raw_density_matrix.csv".format(
                self.output_base, label, 'feature'))
        self.maptype = 'a5ss'
    ...
    def create_mxe_matrices(self, label=""):
        ...
        rbps = [self.ip, self.inp]
        self.logger.info(
            "Start creating the MXE Matrix - {}".format(self.name))
        for i in range(0, len(densities)):
            densities[i]['feature'] = mtx.mutually_exc_exon(
                annotation=self.annotation,
                annotation_type=self.annotation_type,
                density=rbps[i],
                exon_offset=self.exon_offset,
        ...
        self.logger.info(
            "Start creating the MXE Matrix - {}".format(self.name))
        self.ip_raw_density['feature'].to_csv(
            "{}.ip.{}.{}.mxe.raw_density_matrix.csv".format(self.output_base,
                                                            label, 'feature'))
        self.input_raw_density['feature'].to_csv(
            "{}.input.{}.{}.mxe.raw_density_matrix.csv".format(
                self.output_base, label, 'feature'))
        self.maptype = 'mxe'
    ...
    def create_se_matrices(self, label=""):
        ...
        for i in range(0, len(densities)):
            densities[i]['feature'] = mtx.skipped_exon(
                annotation=self.annotation,
                annotation_type=self.annotation_type,
                density=rbps[i],
                exon_offset=self.exon_offset,
                ...
                combine_regions=True)
        self.logger.info(
            "Finished creating the SE Matrix - {}".format(self.name))
        self.ip_raw_density['feature'].to_csv(
            "{}.ip.{}.{}.se.raw_density_matrix.csv".format(self.output_base,
                                                           label, 'feature'))
        self.input_raw_density['feature'].to_csv(
            "{}.input.{}.{}.se.raw_density_matrix.csv".format(self.output_base,
                                                              label,
                                                              'feature'))
        self.maptype = 'se'
    ...
    def create_ri_matrices(self, label=""):
        ...
        for i in range(0, len(densities)):
            densities[i]['feature'] = mtx.retained_intron(
                annotation=self.annotation,
                annotation_type=self.annotation_type,
                density=rbps[i],
                exon_offset=self.exon_offset,
        ...
        self.logger.info(
            "Finished creating the SE Matrix - {}".format(self.name))
        self.ip_raw_density['feature'].to_csv(
            "{}.ip.{}.{}.ri.raw_density_matrix.csv".format(self.output_base,
                                                           label, 'feature'))
        self.input_raw_density['feature'].to_csv(
            "{}.input.{}.{}.ri.raw_density_matrix.csv".format(self.output_base,
                                                              label,
                                                              'feature'))
        self.maptype = 'ri'
    ...
    def create_unscaled_exon_matrices(self, label=""):
        ...
        rbps = [self.ip, self.inp]
        self.logger.info(
            "Start creating the unscaled Matrix - {}".format(self.name))
        for i in range(0, len(densities)):
            densities[i]['feature'] = mtx.create_unscaled_exon_matrix(
                annotation=self.annotation,
                annotation_type=self.annotation_type,
                density=rbps[i],
                exon_offset=self.exon_offset,
