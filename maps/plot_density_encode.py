...
def main(argv=None):  # IGNORE:C0111
    ...
    parser.add_argument("-f", "--flipped", dest="flipped", help="if positive is negative (pos.bw really means neg.bw)",
                        default=False, action='store_true')
    parser.add_argument("-r", "--annotation_dir", dest="annotation_dir",
                        help="annotation directory or testfile (if -t)")
    parser.add_argument("-e", "--event", dest="event",
                        help="event. Can be either: [se, a3ss, a5ss, ri, mxe, cdsstart, cdsend, txstart, txend]")
    parser.add_argument("-t", "--test", dest="test", help="for testing purposes...", default=False, action='store_true')
    parser.add_argument("-a", "--annotation_type", dest="annotation_type", help="annotation type ([miso], xintao, bed)",
                        default='miso')
    parser.add_argument("-exon", "--exon_offset", dest="exon_offset", help="exon offset (default: 50)", default=50,
                        type=int)
    parser.add_argument("-intron", "--intron_offset", dest="intron_offset", help="intron offset (default: 300)",
                        default=300, type=int)
    parser.add_argument("-c", "--confidence", dest="confidence",
                        help="Keep only this percentage of events while removing others as outliers (default 0.95)",
                        default=0.95, type=float)
    parser.add_argument("-five", "--five", dest="five", help="look for .5 extension to prefix", default=False,
                        action='store_true')
    parser.add_argument("-subtract", "--subtract", dest="subtract", help="normalize via subtraction method",
                        default=False, action='store_true')
    parser.add_argument("-entropy", "--entropy", dest="entropy", help="normalize via entropy method", default=False,
                        action='store_true')
    parser.add_argument("-density", "--density", dest="density", help="plot raw density", default=False,
                        action='store_true')
    parser.add_argument("-input", "--input", dest="input", help="plot input density", default=False,
                        action='store_true')
        ...
        for line in f:
            try:
                ...
                if (len(line) == 6):
                    ...
                    inp1 = line[5].replace('/ps-yeolab2/',
                                           '/ps-yeolab3/').strip()  # this may be the last column in the manifest.
                    ...
                    cell_line = line[2]
                    rep1 = line[3].replace('/ps-yeolab2/', '/ps-yeolab3/')  # NOTHING should be in ps-yeolab2
                    inp1 = line[4].replace('/ps-yeolab2/',
                                           '/ps-yeolab3/').strip()  # this may be the last column in the manifest.
                ...
                for i in range(0, len(reps)):
                    ...
                    positive_annotation = os.path.join(annotation_dir, '{}-{}-{}-positive.txt').format(rbp_name,
                                                                                                       cell_line,
                                                                                                       event.upper())
                    negative_annotation = os.path.join(annotation_dir, '{}-{}-{}-negative.txt').format(rbp_name,
                                                                                                       cell_line,
                                                                                                       event.upper())
                    bg_annotation = os.path.join(annotation_dir, '{}-{}-{}.txt').format(rbp_name, cell_line,
                                                                                        event.upper())
                    ...
                    for norm_name, norm_func in normfuncs.iteritems():
                        ...
                        output_filename = os.path.join(outdir, reps[i]) + ".{}.{}.removeoutliers.svg".format(args.event,
                                                                                                             norm_name)
                        ...
                        title = '{}: {} events (keep={})\nincl (n={}), excl (n={})'.format(prefix,
                                                                                           event,
                                                                                           confidence,
                                                                                           len(inclusionClip.density[
                                                                                                   'feature']),
                                                                                           len(exclusionClip.density[
                                                                                                   'feature']))
                        ...
                        if (event == 'a3ss'):
                            ...
                            logger.error(
                                "invalid event (choose a3ss, a5ss, se, ri, cdsstarts, cdsends, txstarts, txends)")
